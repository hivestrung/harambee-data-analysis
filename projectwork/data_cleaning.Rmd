...
ggplot(df3, aes(y = percent, x = numchildren, fill = moreThanSixMths)) + geom_bar(stat="identity")
```
...
```{r}
df3_numearnincome = group_by(df2, numearnincome) %>% count(moreThanSixMths) %>% group_by(numearnincome) %>% mutate(percent = n)
...
age at survey
```{r}
df3_ageatsurvey = ggplot(df2, aes(y=daysWorked, x=mean_num_score)) + geom_point() + geom_count()
```
